...
function renameAccount(taken_name) {
    const newname = window.prompt(
        `${taken_name ? loc("code_changeNameRetry").replace("|NAME|", taken_name) + " " : ""}${loc("code_changeNameConfirm")}`
    );
    if (newname) {
...
function updateLocElements() {
    ...
    window.itemListPromise.then(itemMap => {
        ...
        document.querySelectorAll("[data-loc-itemMap-norm]").forEach(elm => {
...
const icons = {
    ...
    note: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M320 576C461.4 576 576 461.4 576 320C576 178.6 461.4 64 320 64C178.6 64 64 178.6 64 320C64 461.4 178.6 576 320 576zM320 200C333.3 200 344 210.7 344 224L344 336C344 349.3 333.3 360 320 360C306.7 360 296 349.3 296 336L296 224C296 210.7 306.7 200 320 200zM293.3 416C292.7 406.1 297.6 396.7 306.1 391.5C314.6 386.4 325.3 386.4 333.8 391.5C342.3 396.7 347.2 406.1 346.6 416C347.2 425.9 342.3 435.3 333.8 440.5C325.3 445.6 314.6 445.6 306.1 440.5C297.6 435.3 292.7 425.9 293.3 416z"/></svg>`
};
...
dictPromise.then(() => {
    ...
    invenotryHeaders.forEach(elm => {
        elm.innerHTML = "";
        elm.classList.remove(...elm.classList);
        elm.classList.add("card-header", "d-flex", "justify-content-between", "align-items-center");

        const cardType = elm.dataset.cardtype;

        {
            const h5 = document.createElement("h5");
            h5.classList.add("mb-0");
            const locStr = `inventory_${cardType.charAt(0).toLowerCase() + cardType.slice(1)}`;
            h5.textContent = loc(locStr);
            h5.setAttribute("data-loc", locStr);

            elm.appendChild(h5);
        }
        {
            const butDiv = document.createElement("div");
            butDiv.classList.add("d-flex", "gap-2");

            if (!["MoaPets", "Hoverboards", "Boosters"].includes(cardType)) {
                const a = document.createElement("a");
                a.href = "#";
                a.onclick = function (event) {
                    event.preventDefault();
            ...
            elm.appendChild(butDiv);
        }
    });
    const guildHeaders = document.querySelectorAll(".card-header[data-guildCardType]");
    guildHeaders.forEach(elm => {
        elm.innerHTML = "";
        elm.classList.remove(...elm.classList);
        elm.classList.add("card-header", "d-flex", "justify-content-between", "align-items-center");

        const cardType = elm.dataset.guildcardtype;

        {
            const h5 = document.createElement("h5");
            h5.classList.add("mb-0");
            const locStr = `guildView_${cardType.charAt(0).toLowerCase() + cardType.slice(1)}`;
            h5.textContent = loc(locStr);
            h5.setAttribute("data-loc", locStr);
        ...
        }
        {
            ...
            }
            {
                ...
                a.onclick = function (event) {
                    event.preventDefault();
                    if (cardType != "TechProjects") {
                        debounce(bulkRemoveVaultItems, cardType);
                    } else {
...
function addMissingVaultItems(vaultType) {
    ...
    document.querySelectorAll(`#datalist-${datalist} option`).forEach(elm => {
        if (!document.querySelector(`#${vaultType}-list [data-item-type='${elm.getAttribute("data-key")}']`)) {
            let ItemCount = 1;
            if (vaultType == "VaultShipDecorations") ItemCount = 999999;
            requests.push({ ItemType: elm.getAttribute("data-key"), ItemCount });
        }
    });
...
function bulkRemoveVaultItems(vaultType) {
    ...
    document.querySelectorAll(`#datalist-${datalist} option`).forEach(elm => {
        if (document.querySelector(`#${vaultType}-list [data-item-type='${elm.getAttribute("data-key")}']`)) {
            let ItemCount = 1;
            if (vaultType == "VaultShipDecorations") ItemCount = 999999;
            requests.push({ ItemType: elm.getAttribute("data-key"), ItemCount: ItemCount * -1 });
        }
    });
...
function buildMarketMetadataPatch() {
    ...
    return {
        ...
        text: [
            ...
            "# Contains only enabled products in enabled Market tabs.",
...
async function saveMarketMetadataPatch() {
    const patch = buildMarketMetadataPatch();
    if (!patch) return toast(loc("market_metadataPatchEmpty"));
    try {
        ...
        await writable.write(patch.text);
        await writable.close();
        toast(loc("market_metadataPatchSaved").replace("|COUNT|", String(patch.count)));
    } catch (error) {
        if (error?.name != "AbortError") toast(String(error));
    }
}

function buildTennogenCompatibilityPatch() {
    const itemType = marketItemTypes.find(([type]) => type == "Tennogen");
    if (!itemType) return;
    const metadataPaths = [
        ...new Set(
            getMarketItemTypeOptions(itemType)
                .map(option => getMarketStoreItemPath(option.getAttribute("data-key")))
                .filter(Boolean)
        )
    ].sort();
    if (!metadataPaths.length) return;
    const sections = metadataPaths.map(metadataPath =>
        [
            `> ${metadataPath}`,
            "    r|IsSteamPurchase=1|IsSteamPurchase=0",
            "    r|PCProdId=|",
            "    r|AvailableOnPlatform[CP_WINDOWS]=0|AvailableOnPlatform[CP_WINDOWS]=1"
        ].join("\r\n")
    );
    return {
        count: metadataPaths.length,
        text: [
            "# Generated by SpaceNinjaServer for the current Tennogen item list.",
            "# Only removes Tennogen Steam/product/platform restrictions.",
            "# Prices, currencies, listing state, and Market tabs are not changed.",
            "# Re-generate when the server's Tennogen item list changes.",
            "# Save in OpenWF/Metadata Patches and fully restart the game.",
            "",
            sections.join("\r\n\r\n"),
            ""
...
async function saveTennogenCompatibilityPatch() {
    const patch = buildTennogenCompatibilityPatch();
    if (!patch) return toast(loc("market_tennogenCompatibilityPatchEmpty"));
    try {
        if (typeof window.showSaveFilePicker != "function") {
            throw new Error(loc("market_savePickerUnsupported"));
        }
        const fileHandle = await window.showSaveFilePicker({
            suggestedName: "Tennogen patch.txt",
            types: [
                {
                    description: "OpenWF Metadata Patch",
                    accept: { "text/plain": [".txt"] }
        ...
        await writable.write(patch.text);
        await writable.close();
        toast(loc("market_tennogenCompatibilityPatchSaved").replace("|COUNT|", String(patch.count)));
    } catch (error) {
        if (error?.name != "AbortError") toast(String(error));
    }
}
...
function createUpgradeInput(targetDiv, itemMap, itemType, upgrade) {
    ...
    }
    {
        ...
        rangeSuffix.textContent = displayAsPercent ? "%" : itemMap["/Lotus/Language/UiElements/PLACEMENT_METERS"].name;
