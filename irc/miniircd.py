...
class Client:
    ...
    def __init__(self, server: "Server", socket: Socket) -> None:
        ...
        self.reply(b"NOTICE * :Auth C5E20E78:*** skipping identd (disabled by server administrator)..")
    ...
    def __registration_handler(
        ...
        if self.nickname and self.user:
            ...
            self.reply(
                b"005 %s ACCEPT=30 AWAYLEN=200 CALLERID=g CASEMAPPING=rfc1459 CHANLIMIT=#:20 CHANMODES=b,k,l,Dimnpst CHANNELLEN=64 CHANTYPES=# CHARSET=utf-8 ELIST=CMNTU HOSTLEN=64 KEYLEN=32 :are supported by this server.."
                % self.nickname
            )
            self.reply(
                b"005 %s KICKLEN=255 LINELEN=512 MAXLIST=b:4096 MAXTARGETS=20 MODES=20 NAMELEN=128 NETWORK=war-irc NICKLEN=54 PREFIX=(ohv)@%s SAFELIST STATUSMSG=@%s TOPICLEN=307 USERLEN=64 :are supported by this server.."
                % (self.nickname, b"%+", b"%+")
            )
            self.reply(
                b"005 %s USERMODES=,,s,giowx WHOX :are supported by this server.."
                % self.nickname
            )
            self.send_lusers()
            self.send_motd()
            self.reply(
                b"396 %s %s :is now your displayed host.."
                % (self.nickname, b"net-64o.b32.9uu0l2.IP")
            )
    ...
    def send_lusers(self) -> None:
        ...
        self.reply(
            b"252 %s %d :operator(s) online.."
            % (self.nickname, 1)
        )
        self.reply(
            b"253 %s %d :unknown connections.."
            % (self.nickname, 1)
        )
        self.reply(
            b"254 %s %d :channels formed.."
            % (self.nickname, 1)
        )
        self.reply(
            b"255 %s :I have %d clients and %d servers.."
            % (self.nickname, 1, 1)
        )
        self.reply(
            b"265 %s :Current local users: %d  Max: %d.."
            % (self.nickname, 1, 1)
        )
        self.reply(
            b"266 %s :Current global users: %d  Max: %d.."
            % (self.nickname, 1, 1)
        )
    ...
    def send_motd(self) -> None:
        ...
        if motdlines:
            ...
            self.reply(b"422 %s :Message of the day file is missing..." % self.nickname)
